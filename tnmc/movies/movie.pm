...

{
    ...
    sub get_movie_extended2 {
        ...
        foreach my $theatreid (@theatres) {
            ...
            $movie->{'theatres_url'} .=
" <a href=\"http://www.mytelus.com/movies/tdetails.do?theatreID=$theatre->{'mybcid'}\">$theatre->{'name'}</a>";
        }
        ...
        foreach my $userID (keys %$votes) {
            ...
            if ($attend == -1) {
                ...
                if ($type == 1) {
                    ...
                    $votesHTML{$username} =
                      "<b><font style='background-color: #ffff88'>&nbsp;$username&nbsp;</font></b>";
                    $votesText{$username} = "**${username}**";
                    $movie->{votesSuperfave}++;
                }
                elsif ($type == 4) {
                    $votesHTML{$username} =
                      "<b><font style='background-color: #ff88ff'>&nbsp;$username&nbsp;</font></b>";
                    $votesText{$username} = "***${username}***";
                    $movie->{votesBday}++;
                }
            }
